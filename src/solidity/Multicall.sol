...
contract Multicall {
    constructor(address[] memory targets, bytes[] memory data) payable {
        assembly {
            ...
            for {} iszero(eq(dataPtr, dataPtrEnd)) {} {
                ...
                if iszero(call(gas(), mload(targetPtr), 0, add(currentData, 0x20), mload(currentData), 0x00, 0x00)) {
...
contract MulticallSingleTarget {
    constructor(address target, bytes[] memory data) payable {
        assembly {
            let dataLength := mload(data)
            let returndata := mload(0x40) // Point `results` to start of free memory
            let results := add(returndata, 0x20) // Pointer to `results`.
            ...
            for {} iszero(eq(dataPtr, dataPtrEnd)) {} {
                ...
                if iszero(call(gas(), target, 0, add(currentData, 0x20), mload(currentData), 0x00, 0x00)) {
                    // Bubble up the revert if the delegatecall reverts.
                    returndatacopy(0x00, 0x00, returndatasize())
                    revert(0x00, returndatasize())
                }
...
contract MulticallRef {
    constructor(address[] memory targets, bytes[] memory datas) payable {
